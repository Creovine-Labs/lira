...
Escalation is *implicit*: `voiceReplyIndicatesUnknown()` (lines 598–629) detects "I don't know"-shaped phrases in Lira's output and silently routes the conversation to the human queue via `escalationService.escalateConversation()`. No confirm flow, no tool invocation.
...
export interface SyntheticNimbusProfile {
  // existing fields
  invoices?: Array<{ id: string; date: string; amount_usd: number; status: string; customer: string }>
  expenses?: Array<{ id: string; category: string; vendor: string; amount_usd: number; date: string }>
...
if (profile.invoices?.length) {
  ...
  lines.push(`  - Recent invoices: ${recent.map(i => `#${i.id} ${i.customer} $${i.amount_usd} ${i.status}`).join('; ')}`)
}
```
...
The agent reasons: *"Customer asks about a refund → kb_search policy → get_invoices to find the one in question → stripe_refund_charge with confirm gate."*
...
```ts
const dashboardSummary = customer
  ? await fetchDashboardContext(orgId, customer.customer_id)
  : null
...
*— End of audit. Update when Nova Sonic model rolls forward, agent model upgrades, or KB stack changes.*
