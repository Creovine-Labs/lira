        ...
        {!success ? (
          <>
            <header className="tpm-header">
              ...
              <p className="tpm-meta">
                This is a demo of Lira — the AI behind Nimbus support. Spin up a quick test profile
                so Lira knows who you are when you chat with her.
              </p>
            </header>
            ...
            >
              ...
              <label className="tpm-field">
                <span className="tpm-label">
                  Any open issues you want Lira to know about?{' '}
                  <span className="tpm-optional">(optional)</span>
                </span>
                <textarea
                  rows={2}
                  maxLength={280}
              ...
              <div className="tpm-actions">
                <button type="submit" className="tpm-submit" disabled={submitting || !name.trim()}>
                  {submitting ? 'Creating…' : 'Create test profile'}
                </button>
                <button type="button" className="tpm-skip" onClick={onClose} disabled={submitting}>
                  Skip — just open chat
                </button>
              </div>

              <p className="tpm-disclosure">
                This profile is fictional, stored only for this session, and deleted in 24 hours. No
                real signup. No password. No real account data.
              </p>
            </form>
          </>
        ) : (
          <div className="tpm-success">
            ...
            <p className="tpm-success-meta">
              Your test Nimbus profile is set up on the <strong>{success.plan_label}</strong> plan.
              Lira knows your account — your last invoice was ${success.last_invoice_amount_usd} on{' '}
              {success.last_invoice_date}, and your next one is due {success.next_invoice_date}.
            </p>
