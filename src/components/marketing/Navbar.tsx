...
interface MarketingNavbarProps {
  ...
  /** Pixels to push the header down by — used when a full-width announcement
   *  bar is pinned above it. */
  topOffset?: number
}
...
export function MarketingNavbar({ variant = 'light', topOffset = 0 }: MarketingNavbarProps) {
  ...
  const shellClass =
    isOverlay && !scrolled
      ? 'absolute inset-x-0 z-[777] text-white'
      : 'fixed inset-x-0 z-[777] border-b border-gray-200/70 bg-[#fbfaf6]/90 text-gray-900 shadow-sm backdrop-blur-md'

  const linkClass =
    isOverlay && !scrolled ? 'text-white/88 hover:text-white' : 'text-gray-700 hover:text-gray-950'

  return (
    <header className={shellClass} style={{ top: topOffset }}>
