...
export function DemoNimbusDashboard({ profile, onSignOut }: DemoNimbusDashboardProps) {
  ...
  const revenueSeries = useMemo(
    () => generateRevenueSeries(seed, planAmount),
    [seed, planAmount],
  )
  ...
  const customerCount = useMemo(
    () => 12 + (hashFromString(seed + 'cust') % 36),
    [seed],
  )
    ...
    <div className="ndb-root">
      ...
      <header className="ndb-topbar">
        ...
        <div className="ndb-top-actions">
          ...
          <button type="button" className="ndb-icon-btn ndb-icon-btn-bell" aria-label="Notifications">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="ndb-user-wrap">
            ...
            >
              ...
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ndb-chevron">
          ...
          <div className="ndb-side-foot">
            <div className="ndb-plan-mini">
              ...
              {profile.subscription_status === 'canceled' ? (
                <div className="ndb-plan-mini-canceled">Canceled · ends {fmtDate(profile.next_invoice_date)}</div>
              ) : (
                <div className="ndb-plan-mini-meta">Renews {fmtDate(profile.next_invoice_date)}</div>
              )}
              <button
                type="button"
    ...
    <>
        ...
        <div className="ndb-plan-badge">
          ...
          <div className="ndb-plan-badge-name">
            ...
            <span className="ndb-plan-badge-price">
              ${profile.last_invoice_amount_usd}<span>/mo</span>
            </span>
          </div>
          <button type="button" className="ndb-secondary-btn ndb-plan-badge-btn" onClick={onChangePlan}>
            Change plan
          </button>
        </div>
      </div>
      ...
      <section className="ndb-prompt">
        ...
        <p>
          ...
          <em>"how do I add a new customer?"</em>, or <em>"refund my last payment"</em> in the
          chat bubble.
        </p>
      </section>
    </>
    ...
    <>
      ...
      <section className="ndb-kpis">
        <div className="ndb-kpi">
          ...
          <div className="ndb-kpi-trend is-up">{invoices.filter((i) => i.status === 'Paid').length} invoices</div>
        </div>
        <div className="ndb-kpi">
          <div className="ndb-kpi-label">Pending</div>
          <div className="ndb-kpi-value">{fmtUSD(totals.Pending)}</div>
          <div className="ndb-kpi-trend">{invoices.filter((i) => i.status === 'Pending').length} invoices</div>
        </div>
        <div className="ndb-kpi">
          <div className="ndb-kpi-label">Overdue</div>
          <div className="ndb-kpi-value">{fmtUSD(totals.Overdue)}</div>
          <div className="ndb-kpi-trend is-down">{invoices.filter((i) => i.status === 'Overdue').length} need follow-up</div>
        </div>
        <div className="ndb-kpi">
      ...
      <section className="ndb-card">
        <div className="ndb-toolbar">
          ...
          <div className="ndb-toolbar-right">
            ...
            />
            <button type="button" className="ndb-primary-btn">+ New invoice</button>
          </div>
        </div>
        ...
        <div className="ndb-table ndb-table-wide">
          ...
          {filtered.map((inv) => (
            <div key={inv.id} className="ndb-table-row">
              ...
              <span>
                <button type="button" className="ndb-row-btn">…</button>
              </span>
            </div>
          ))}
          {filtered.length === 0 && (
            <div className="ndb-empty">No invoices match this filter.</div>
          )}
        </div>
      </section>
    </>
    ...
    <>
      ...
      <section className="ndb-card">
        <div className="ndb-toolbar">
          ...
          <div className="ndb-toolbar-right">
            <button type="button" className="ndb-primary-btn">+ Add expense</button>
          </div>
        </div>
        <div className="ndb-table ndb-table-wide">
          <div className="ndb-table-head">
...
function ReportsView({
  ...
  invoices
    ...
    .forEach((i) => {
      ...
      if (daysOverdue <= 30) aging['0-30'] += i.amount
      else if (daysOverdue <= 60) aging['31-60'] += i.amount
      else if (daysOverdue <= 90) aging['61-90'] += i.amount
      else aging['90+'] += i.amount
    })
    ...
    <>
      ...
      <section className="ndb-card">
        <div className="ndb-card-head">
          ...
          <div className="ndb-toolbar-right">
            <button type="button" className="ndb-secondary-btn">Export CSV</button>
            <button type="button" className="ndb-secondary-btn">Export PDF</button>
          </div>
        </div>
          ...
          {revenueSeries.map((m) => {
            ...
            return (
              <div key={m.month} className="ndb-chart-col">
                <div className="ndb-chart-bar" style={{ height: `${h}px` }} title={fmtUSD(m.revenue)} />
      ...
      <div className="ndb-two-col">
        ...
        <section className="ndb-card">
            ...
            {(['0-30', '31-60', '61-90', '90+'] as const).map((bucket) => {
              ...
              return (
                <li key={bucket} className="ndb-expense-row">
                  <span className="ndb-expense-icon" aria-hidden="true">📅</span>
          ...
          <p className="ndb-note">
            <strong>{fmtUSD(aging['90+'])}</strong> over 90 days. Ask Lira to draft follow-up
            emails for these.
          </p>
        </section>
      </div>
      ...
      <section className="ndb-card">
        <div className="ndb-card-head">
          ...
          <span className="ndb-card-sub">Subscription status across {customers.length} customers</span>
        </div>
    ...
    <>
      ...
      <section className="ndb-kpis">
        ...
        <div className="ndb-kpi">
          ...
          <div className="ndb-kpi-value">{customers.filter((c) => c.status === 'Active').length}</div>
          <div className="ndb-kpi-trend">paying</div>
        </div>
        <div className="ndb-kpi">
          <div className="ndb-kpi-label">Trialing</div>
          <div className="ndb-kpi-value">{customers.filter((c) => c.status === 'Trialing').length}</div>
          <div className="ndb-kpi-trend">no card</div>
        </div>
        <div className="ndb-kpi">
          <div className="ndb-kpi-label">Past due</div>
          <div className="ndb-kpi-value">{customers.filter((c) => c.status === 'Past due').length}</div>
        ...
        <div className="ndb-toolbar">
          ...
          <div className="ndb-toolbar-right">
            <input
              ...
              className="ndb-search"
            />
            <button type="button" className="ndb-primary-btn">+ Add customer</button>
          </div>
        </div>
              ...
              <div key={i.name} className="ndb-integ-row">
                ...
                <div className="ndb-integ-row-right">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setToggled((prev) => ({ ...prev, [i.name]: !prev[i.name] }))
                    }
                    aria-pressed={isOn}
    ...
    <>
      ...
      <section className="ndb-card">
        ...
        <div className="ndb-settings-grid">
          <div className="ndb-settings-field">
            ...
            <input type="text" defaultValue={profile.name} className="ndb-input" />
          </div>
          <div className="ndb-settings-field">
          ...
          <div className="ndb-settings-field">
            <label>Company</label>
            <input type="text" defaultValue="Nimbus Finance, Inc." className="ndb-input" />
          </div>
          <div className="ndb-settings-field">
            <label>Time zone</label>
            <select className="ndb-input">
            ...
            <div className="ndb-plan-actions">
              ...
              {canceled ? (
                ...
                <button
                  ...
                  className="ndb-secondary-btn"
                  onClick={() => {
                    if (window.confirm('Cancel your subscription? Access ends at the current billing period.')) {
      ...
      <section className="ndb-card">
        ...
        <div className="ndb-api-row">
          ...
          <button type="button" className="ndb-danger-btn">Rotate</button>
        </div>
      </section>
      ...
      <section className="ndb-card ndb-card-danger">
        ...
        <div className="ndb-danger-row">
          <div>
            ...
            <div className="ndb-danger-sub">Permanently delete this Nimbus account and all data.</div>
          </div>
          <button
            type="button"
            className="ndb-danger-btn"
            onClick={() => window.alert('In a real Nimbus account this would delete all your data. In the demo it stays put.')}
          >
            Delete account
          </button>
        </div>
...
function StatusPill({ status }: { status: Invoice['status'] }) {
  return (
    <span className={`ndb-pill ndb-pill-${status.toLowerCase()}`}>{status}</span>
  )
}

function CustomerStatusPill({ status }: { status: Customer['status'] }) {
  const klass =
    status === 'Active' ? 'ndb-pill-paid'
      : status === 'Trialing' ? 'ndb-pill-pending'
      : status === 'Past due' ? 'ndb-pill-overdue'
      : 'ndb-pill-muted'
...
function UpgradePlanModal({
  ...
  // Sync the selected pill with currentPlan whenever the modal opens or the
  // current plan changes underneath us. Without this, `useState` only takes
  // its initial value once, so re-opening the modal after an upgrade would
  ...
  return (
    <div className="ndb-modal-backdrop" role="presentation" onClick={(e) => {
      ...
      <div className="ndb-modal" role="dialog" aria-modal="true" aria-labelledby="upgrade-title">
        <header className="ndb-modal-head">
          ...
          <button type="button" className="ndb-modal-close" onClick={onClose} aria-label="Close">×</button>
        </header>
...
function UpdateCardModal({
  ...
  return (
    <div className="ndb-modal-backdrop" role="presentation" onClick={(e) => {
      ...
      <div className="ndb-modal ndb-modal-narrow" role="dialog" aria-modal="true" aria-labelledby="card-title">
        <header className="ndb-modal-head">
          <div>
