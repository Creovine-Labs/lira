...
const _wavRecorderProto = WavMediaManager.prototype as unknown as {
...
const _wrappedInit = (WavMediaManager.prototype as unknown as { initialize: () => Promise<void> })
  .initialize
;(
  WavMediaManager.prototype as unknown as {
    initialize: () => Promise<void>
...
export class PipecatTransport {
  ...
  async start(): Promise<void> {
    ...
    if (enableMic) {
      ...
      try {
        ...
        console.log(`[Lira voice] AudioContext OK: state=${ctx.state} sampleRate=${ctx.sampleRate}`)
  ...
  async signalClientReady(): Promise<void> {
    ...
    const c = this.client as unknown as {
      sendClientMessage?: (t: string, d: unknown) => Promise<void>
    }
