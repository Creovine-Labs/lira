      ...
      {snapshot && !snapshot.error && (
        <>
          ...
          <section className="mt-6 rounded-xl border border-gray-200 bg-white p-5">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-4">
              ...
              <Stat
                ...
                emphasis="warning"
              />
              <Stat label="Voice sub-budget" value={`$${snapshot.voice_sub_budget_usd ?? 0}`} />
              <Stat label="Hard cap" value={`$${snapshot.hard_cap_usd ?? 0}`} emphasis="danger" />
            </dl>
          </section>
          ...
          <footer className="mt-6 text-xs text-gray-400">
            ...
            {lastFetched && <span> · Last refreshed {lastFetched.toLocaleTimeString()}</span>}
          </footer>
        </>
      )}
    </div>
...
function SpendCard({
  ...
  const pct =
    cap && cap > 0 && typeof value === 'number' ? Math.min(100, (value / cap) * 100) : null
...
function CountCard({
  ...
  return (
    <div className="rounded-xl border border-violet-200 bg-violet-50 p-4">
      <div className="text-xs font-medium uppercase tracking-wider text-violet-700">{label}</div>
